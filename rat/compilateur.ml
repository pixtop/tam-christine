...
(* + affichage du placement des variables *)

open PasseTdsRat
open PasseTypeRat
open PassePlacementRat
module AfficherDep = Compilateur (PasseTdsRat) (PasseTypeRat) (PassePlacementRat) (PasseAffichePlacement)
...
(* filename : l'URL du fichier non conforme *)
...
(* Paramètre ratfile : l'URL du fichier rat à compiler *)
...
(* afficherDep : string -> string *)
(* Analyse un code rat et renvoie le placement mémoie des variables *)
(* Paramètre ratfile : l'URL du fichier rat à compiler *)
(* Utilise le compilateur "AfficherDep" pour renvoyer le placement mémoire des variables *)
(* Erreur si soucis lors de l'analyse lexicale, l'analyse syntaxique,
 mauvaise utilisation des identifiants ou soucis de typage *)

let afficherDep ratfile =
  let input = open_in ratfile in
  let filebuf = Lexing.from_channel input in
  try
    let ast = Parser.main Lexer.token filebuf in
